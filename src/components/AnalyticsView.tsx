...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<'2024-Q1' | '2024-Q2' | '2024-All'>('2024-Q2');
  ...
  const handleExportPDF = () => {
    exportTransactionsToPDF(transactions, school, 'LAPORAN STATISTIK & SIRKULASI PERPUSTAKAAN');
  };

  const handleExportCSV = () => {
    const csvData = monthlyCirculationData.map(d => ({
      Bulan: d.month,
      Jumlah_Peminjaman: d.borrows,
      Jumlah_Pengembalian: d.returns,
      Sirkulasi_Total: d.borrows + d.returns
    }));
    exportToCSV(csvData, 'Rekap_Statistik_Sirkulasi_Lumina');
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400 font-mono">Tahun Ajaran 2023/2024</span>
          </div>
          <h2 className="text-xl font-bold text-white tracking-tight">Analitik Sirkulasi & Minat Baca</h2>
          <p className="text-xs text-slate-400 mt-0.5">Pantau tren peminjaman, buku terfavorit, rasio keterlambatan, dan ekspor data resmi</p>
        </div>
